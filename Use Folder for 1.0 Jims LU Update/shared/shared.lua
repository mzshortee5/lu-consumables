...
function toggleItem(give, item, amount)
    if giveItem then
        -- This will only allow them to get the item if everything is correct
        TriggerServerEvent("lu-consumables:server:toggleItem", give, item, amount)
    else
        -- This will cancel it and log everything, maybe they are cheating or something?
        TriggerServerEvent("lu-consumables:server:CancelAction", give, item, amount)
    end
end
...
--Screen Effects
local acidEffect = false
function AcidEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if acidEffect then return else acidEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
	...
	-- end
    acidEffect = false
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2stopped") end
...
local midredEffect = false
function MidredEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if midredEffect then return else midredEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
...
local midgreenEffect = false
function MidgreenEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if midgreenEffect then return else midgreenEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
...
local midblueEffect = false
function MidblueEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if midblueEffect then return else midblueEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
...
local noirEffect = false
function NoirEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if noirEffect then return else noirEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
...
local shortredEffect = false
function ShortredEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if shortredEffect then return else shortredEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
...
local shortgreenEffect = false
function ShortGreenEffect()
        local ScreenColour = ""
        local ped = PlayerPedId()
    if shortgreenEffect then return else shortgreenEffect = true end
    if Config.Debug then print("^5Debug^7: ^3CustomEffect^7() ^2activated") end
...
function NightVisionEffect()
    ...
    SetNightvision(true)
	Wait(math.random(Config.Effectmin, Config.Effectmax))  -- FEEL FREE TO CHANGE THIS
