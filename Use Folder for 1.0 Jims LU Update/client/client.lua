...
local function syncConsumables()
	local p = promise.new() QBCore.Functions.TriggerCallback('lu-consumables:server:syncConsumables', function(cb) p:resolve(cb) end) Consumables = Citizen.Await(p)
	local p2 = promise.new() QBCore.Functions.TriggerCallback('lu-consumables:server:syncEmotes', function(cb) p2:resolve(cb) end) Emotes = Citizen.Await(p2)
    if Config.Debug then print("^5Debug^7: ^2Retrieved ^6"..countTable(Consumables).." ^2Items and ^6"..countTable(Emotes).." ^2Emotes^7") end
    propConvert()
end

RegisterNetEvent("lu-consumables:client:syncConsumables", function(NewConsumables)
...
RegisterNetEvent("lu-consumables:client:syncEmotes", function(NewEmotes)
...
RegisterNetEvent('lu-consumables:Consume', function(itemName)
    ...
    if Config.UseProgbar then
        if Config.ProgressBar == "ox" then
            ...
            CreateThread(function() QBCore.Functions.Progressbar('lumy_consume_', string..QBCore.Shared.Items[itemName].label.."..", time, false, false, {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true,}, {}, {}, {}, function() consuming = false end, function() end, itemName) end)
        end
    else
	...
	if not cancelled then
        ...
        if stats then
            if stats.hunger then TriggerServerEvent("lu-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["hunger"] + stats.hunger, "hunger") end
            if stats.thirst then TriggerServerEvent("lu-consumables:server:addNeed", QBCore.Functions.GetPlayerData().metadata["thirst"] + stats.thirst, "thirst") end
        end
        ...
        if stats then
            if stats.screen then -- Screen effect activation
				...
				if stats.screen == "alien" then CreateThread(function() AlienEffect() end) end
				if stats.screen == "mildred" then CreateThread(function() MildredEffect() end) end
				if stats.screen == "mildgreen" then CreateThread(function() MildgreenEffect() end) end
				if stats.screen == "mildblue" then CreateThread(function() MildblueEffect() end) end
				if stats.screen == "shortgreen" then CreateThread(function() ShortgreenEffect() end) end
				if stats.screen == "shortred" then CreateThread(function() ShortredEffect() end) end
				if stats.screen == "noir" then CreateThread(function() NoirEffect() end) end
				if stats.screen == "acid" then CreateThread(function() AcidEffect() end) end
            end
			if stats.canOD then
			...
			if stats.effect == "kill" then
				SetEntityHealth(PlayerPedId(), GetEntityHealth(Player) - 100)
			end
